...
class Admin_API {
	...
	public function wpforms_submit_action($fields, $entry) {
		$announcement = IAnnouncement::fromForm($fields);
		Database::instance()->create_announcement($announcement);

		// // Debug output $message of all function arguments
		// $message = var_dump($announcement) . '\n';
		// fwrite(fopen('/Users/mat/Downloads/bettenboerse.log', 'a'), $message);
	}
	...
	public function display_field($data = array(), $post = null, $echo = true) {
		...
		switch ($field['type']) {
			...
			case 'image':
				...
				$html .= '<input id="' . $option_name . '_button" type="button" data-uploader_title="' . __('Upload an image', 'bettenboerse') . '" data-uploader_button_text="' . __('Use image', 'bettenboerse') . '" class="image_upload_button button" value="' . __('Upload new image', 'bettenboerse') . '" />' . "\n";
				$html .= '<input id="' . $option_name . '_delete" type="button" class="image_delete_button button" value="' . __('Remove image', 'bettenboerse') . '" />' . "\n";
